...
const { getMessageList, changeToIsRead } = require("../../controller/message");
const { getCartList } = require("../../controller/cart");
const { getBegCartList } = require("../../controller/begCart");
const { getMyBoughtsList } = require("../../controller/myBought");
const { getMyBeggedsList } = require("../../controller/myBegged");
/**
 * 获取登录信息
...
router.get("/", async (ctx, next) => {
  ...
  await ctx.render(
    "index",
    Object.assign(getLoginInfo(ctx), {
...
router.get("/good/edit/:id", async (ctx, next) => {
  const typeResult = await getGoodTypesList();
  const goodId = ctx.request.url.split("/")[
    ctx.request.url.split("/").length - 1
  ];
...
router.get("/beg/edit/:id", async (ctx, next) => {
  const typeResult = await getGoodTypesList();
  const begId = ctx.request.url.split("/")[
    ctx.request.url.split("/").length - 1
  ];
